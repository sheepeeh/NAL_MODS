	...
	def build_csv(target_dir)
		...
		Dir.glob("#{target_dir}/*.xml").each do |xml_file|
			...
			xmldoc.elements.each("mods/originInfo") do |e|
				...
				origin.vals2omeka(@origins)

				copyright = ModsOrigin::Copyright.new
				copyright.get_date(e)
				copyright.vals2omeka(@date_copyright)

				created = ModsOrigin::Created.new
				created.get_date(e)
				created.vals2omeka(@date_created)

				issued = ModsOrigin::Issued.new
				issued.get_date(e)
				issued.vals2omeka(@date_issued)
			end
			...
			xmldoc.elements.each("mods/subject/topic") do |e|
				...
				subj_topic.vals2omeka(@subj_topics)
			end
			...
			xmldoc.elements.each("mods/subject") do |e|
				...
				subj_genre.vals2omeka(@subj_genres)
			end
			...
			xmldoc.elements.each("mods/subject") do |e|
				geo = ModsSubject.new
				geo.get_geo(e)
				geo.vals2omeka(@subj_geos)
			end

			xmldoc.elements.each("mods/subject") do |e|
				time = ModsSubject.new
				time.get_time(e)
				time.vals2omeka(@subj_times)
			end
			...
			xmldoc.elements.each("mods/tableOfContents") do |e|
				toc = ModsToc.new
				toc.get_toc(e)
				toc.vals2omeka(@table_of_contents)
			end
